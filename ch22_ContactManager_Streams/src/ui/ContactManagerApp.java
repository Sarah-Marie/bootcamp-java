...
System.out.println("Welcome to the Contact Manager App!");
		...
		//not in the book- an easy way to print everything in your list:
		...
		System.out.println("Transfrom contact list to list of names:");
		List<String> contactNames = contacts.stream().map(c -> c.getName())
				                                     .collect(Collectors.toList());
		
		contactNames.stream().forEach(str -> System.out.println(str));
		
		System.out.println("Transfrom contact list to list of names - double colon:");
